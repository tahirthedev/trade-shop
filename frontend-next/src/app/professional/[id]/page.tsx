  ...
  const loadProfessional = async () => {
    try {
      ...
      if (response.success && response.professional) {
        ...
        setError(t('professionalProfile.notFound'));
      }
    } catch (err: any) {
      setError(err.message || t('professionalProfile.errorLoading'));
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">{t('professionalProfile.notFound')}</h2>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 py-8">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        >
          ...
          {t('professionalProfile.backToMarketplace')}
        </Button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                ...
                {t('professionalProfile.aiScore.title')}
              </h2>
              <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg p-6">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <div className="text-xl font-semibold">
                      {professional.aiScore.total >= 9 ? t('professional.card.aiScore.elite') :
                       professional.aiScore.total >= 8 ? t('professional.card.aiScore.excellent') :
                       professional.aiScore.total >= 7 ? t('professional.card.aiScore.great') :
                       professional.aiScore.total >= 6 ? t('professional.card.aiScore.good') :
                       t('professional.card.aiScore.new')}
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm opacity-90 mb-1">{t('professionalProfile.aiScore.skillVerification')}</p>
                  ...
                  <div>
                    <p className="text-sm opacity-90 mb-1">{t('professionalProfile.aiScore.reliability')}</p>
                    <div className="bg-white/20 rounded-full h-2">
                      <div 
                        className="bg-white rounded-full h-2" 
                  ...
                  <div>
                    <p className="text-sm opacity-90 mb-1">{t('professionalProfile.aiScore.quality')}</p>
                    <div className="bg-white/20 rounded-full h-2">
                      <div 
                        className="bg-white rounded-full h-2" 
                  ...
                  <div>
                    <p className="text-sm opacity-90 mb-1">{t('professionalProfile.aiScore.safety')}</p>
                    <div className="bg-white/20 rounded-full h-2">
                      <div 
                        className="bg-white rounded-full h-2" 
              ...
              <p className="text-sm text-gray-600 mt-4">
                {professional.stats.reviewCount} {t('professionalProfile.verifiedReviews')}
              </p>
            </Card>
            ...
            {professional.specialties && professional.specialties.length > 0 && (
              <Card>
                <h2 className="text-2xl font-bold text-gray-900 mb-4">{t('professionalProfile.specialties')}</h2>
            ...
            {professional.certifications && professional.certifications.length > 0 && (
              <Card>
                <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                  ...
                  {t('professionalProfile.certifications')}
                </h2>
                  ...
                  {professional.certifications.map((cert: any, index: number) => (
                    <div key={index} className="border-l-4 border-blue-500 pl-4 py-2">
                      ...
                      <p className="text-sm text-gray-500">
                        {t('professionalProfile.issued')}: {new Date(cert.dateObtained).toLocaleDateString()}
                      </p>
                    </div>
                  ))}
                </div>
            ...
            {professional.portfolio && professional.portfolio.length > 0 && (
              <Card>
                <h2 className="text-2xl font-bold text-gray-900 mb-4">{t('professionalProfile.portfolio')}</h2>
          ...
          <div className="space-y-6">
            {/* Pricing */}
            <Card>
              <h2 className="text-xl font-bold text-gray-900 mb-4">{t('professionalProfile.pricing')}</h2>
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg p-4 mb-4">
                <p className="text-sm text-gray-600 mb-2">{t('professionalProfile.hourlyRate')}</p>
                <p className="text-3xl font-bold text-green-700">
                  ${professional.hourlyRate?.min}-${professional.hourlyRate?.max}
                </p>
                <p className="text-xs text-gray-500 mt-1">{t('professionalProfile.perHour')}</p>
              </div>
            ...
            {/* Contact */}
            <Card>
              <h2 className="text-xl font-bold text-gray-900 mb-4">{t('professionalProfile.contactInfo')}</h2>
              <div className="space-y-3">
                ...
                {!professional.contactPreferences?.phoneVisible && (
                  <div className="text-sm text-gray-500 italic p-3 bg-gray-50 rounded">
                    {t('professionalProfile.phoneHidden')}
                  </div>
                )}
              </div>
            </Card>
            ...
            {professional.languages && professional.languages.length > 0 && (
              <Card>
                <h2 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                  ...
                  {t('professionalProfile.languages')}
                </h2>
                <div className="flex flex-wrap gap-2">
            ...
            {(professional.website || professional.linkedin || professional.twitter) && (
              <Card>
                <h2 className="text-xl font-bold text-gray-900 mb-4">{t('professionalProfile.links')}</h2>
            ...
            {professional.insurance && professional.insurance.provider && (
              <Card>
                <h2 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                  ...
                  {t('professionalProfile.insurance.title')}
                </h2>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-gray-600">{t('professionalProfile.insurance.provider')}:</span>
                    <span className="font-semibold text-gray-900">{professional.insurance.provider}</span>
                  </div>
                  {professional.insurance.coverage && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">{t('professionalProfile.insurance.coverage')}:</span>
            ...
            >
              {t('professionalProfile.requestQuote')}
            </Button>
          </div>
        </div>
      </div>
